...
typedef enum {
    ...
    CUSTOME_KEYS_TYPE /*!< KEYSCAN custome keys keyboard */
} keyscan_keys_type_t;
...
#define KEYSCAN_DEFAULT_CONFIG                                                                                    \
    {                                                                                                             \
        .direction = ROW_SCAN_DIR, .scan_mode = HAL_KEYSCAN_MODE_0, .pulse_time = EVERY_ROW_PULSE_10_US,          \
        .wait_time = WAITING_30_AND_5_US, .idle_time = IDLE_3_MS_AFTER_SCAN, .defence_time = SAMPLE_NONE_DEFENCE, \
        .select_num = SELECT_SIX_KEYS_PER_SCAN, .ghost_check = false, .io_de = true,                              \
    }
